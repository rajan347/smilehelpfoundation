		...
		<table class="table table-striped- table-bordered table-hover table-checkable display" id="m_table_1">
			...
			<?php $i=1; ?>
				...
				<tr>
					...
					<td><img src='public/account/{{$causess->image}}' style="height:50px;width:50px;"></td>
					<td>{{$causess->description}}</td>
					<td>{{$causess->description2}}</td>
					<td>{{$causess->description3}}</td>
					<td>{{$causess->tags}}</td>
					<td><a style="background-color: red;color: white" href="{{ url('deleteblog/'.$causess->id) }}"  class="btn m-btn--pill    btn-secondary m-btn m-btn--custom m-btn--label-brand m-btn--bolder btn_2">Delete</a></td>
					<!-- <td></td> -->
