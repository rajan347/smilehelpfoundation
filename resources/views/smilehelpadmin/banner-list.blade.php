...
@endif
	...
	<div class="m-portlet__body" >
		<table class="table table-striped- table-bordered table-hover table-checkable display" id="m_table_1">
			...
			<tbody>
			<?php $i=1; ?>
				@foreach($banner as $causess)
				<tr>
					<td>{{$causess->id}}</td>
					
					<td>{{$causess->heading}}</td>
					<td><img src='public/account/{{$causess->img}}' style="height:50px;width:50px;"></td>
					<td><a style="background-color: red;color: white" href="{{ url('deletebanner/'.$causess->id) }}"  class="btn m-btn--pill    btn-secondary m-btn m-btn--custom m-btn--label-brand m-btn--bolder btn_2">Delete</a></td>
					<!-- <td></td> -->
